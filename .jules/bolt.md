...
## 2026-07-26 - [Avoid redundant array slice view allocation]
**Learning:** When performing multiple in-place operations on an array slice in a hot path (e.g., `np.subtract(..., out=arr[1:])` followed by `arr[1:] *= dt_inv`), Python creates a new array view object each time `arr[1:]` is evaluated. This redundant object allocation incurs measurable overhead in frequently called inner loops.
**Action:** Pre-allocate the view to a variable (e.g., `v_slice = arr[1:]`) and reuse it across multiple in-place operations to avoid repeated view allocation overhead.
