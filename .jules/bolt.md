...
**Learning:** Adding constraints individually or element-by-element inside tight Python loops to Drake's `MathematicalProgram` is a performance anti-pattern. The allocation overhead for continuously creating expression objects and solver bindings (e.g., via `AddLinearEqualityConstraint` or `AddBoundingBoxConstraint`) can dominate solver setup time. Utilizing matrix-vector structures (like `Ax = b`) allows adding constraints in bulk (`prog.AddLinearEqualityConstraint(A, b, vars_all[k])`), resulting in a ~3x performance improvement.
