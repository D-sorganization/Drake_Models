...
class TestCylinderInertia:
    ...
    def test_axial_less_than_transverse(self):
        ixx, iyy, izz = cylinder_inertia(5.0, 0.1, 1.0)
        assert izz < ixx
...
class TestParallelAxisShift:
    ...
    def test_shift_increases_inertia(self):
        inertia = (1.0, 1.0, 1.0)
...
class TestRotationMatrices:
    def test_x_rotation_identity(self):
        r = rotation_matrix_x(0)
        np.testing.assert_allclose(r, np.eye(3), atol=1e-12)

    def test_y_rotation_identity(self):
        r = rotation_matrix_y(0)
        np.testing.assert_allclose(r, np.eye(3), atol=1e-12)

    def test_z_rotation_identity(self):
